package  {
	...
	public class takeRightOne extends MovieClip {
		var parnt:MovieClip;
		var swid:Number;
		var shit:Number;
		var objs:Array = new Array();// ["brrom", "bucket", "dustpan", "floor_Buffer", "paper_Towel", "vacuum_Cleaner", "waste_Container", "water_Hoses"];
		var answ:Array = new Array();// ["paper_Towel", "water_Hoses" ];
		var titl:Array = ["qs_title30"];
		...
		public function takeRightOne(par:MovieClip, list:Array, num:int, pnum:int) {
			parnt = par;
			swid = par.swid;
			shit = par.shit;
			objs = list.concat();
		...
		function checkForAnswer():Boolean {
					...
					if (obj.x == tar[0] && obj.y == tar[1]) {
						...
						//num++;
							for each(var res in result) {
								if(res[0]==tar[2]) res[1] = true;
							}
						}
					}
				}
			}
			for each(var rs in result) {
				if(!rs[1]){ 
					done = false;
					break;
				}
			...
			if(done) youAreRight();
			else if(num==tarLObj.length) youAreWrong();
			return done;
		}
		function createPosition(ary:Array) {
			...
			for(var a:int = 0; a<ary.length; a++) {
				...
				inst.addChild(ract);
				ract.y = -75;
				result.push([ary[a], false]);
				tarLObj.push(inst);
				tarPos.push([inst.x, inst.y-75, ary[a]]);
				anim.from(inst, 0.5, {scaleX:0, scaleY:0, ease: Elastic.easeOut.config(1, 0.75)});
			}
		}		
		function randomSelect(targ:Array, siz:Number):Array {
